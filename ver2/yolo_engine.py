...
def is_tensorrt_engine(model_path: str) -> bool:
    return str(model_path).lower().endswith(".engine")
...
class YoloEngine(QObject):
    ...
    def _get_model(self, model_path: str, logger: Optional[Logger]) -> YOLO:
        with self._lock:
            ...
            trt = is_tensorrt_engine(model_path)
            if logger:
                kind = "TensorRT engine" if trt else "PyTorch weights"
                logger.info(f"[YOLO] Loading {kind}: {model_path}")
            ...
            if not trt:
                ...
                logger.info("[YOLO] OK TensorRT engine loaded (FP16 baked in — skip PyTorch fuse/half)")
    ...
    def _process_batch(self, jobs: list[Dict[str, Any]]) -> None:
        ...
        if torch.cuda.is_available() and not is_tensorrt_engine(model_path):
...
def log_yolo_device_status(logger: Optional[Logger] = None) -> str:
    """Ghi trạng thái GPU/CPU cho YOLO — đối chiếu bảng log trong README."""
    if torch.cuda.is_available():
        try:
            gpu_name = torch.cuda.get_device_name(0)
            vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024 ** 3)
            msg = f"[YOLO] OK CUDA {gpu_name} ({vram_gb:.1f} GB VRAM)"
        except Exception:
            msg = "[YOLO] OK CUDA enabled"
    else:
        msg = "[YOLO] WARN CPU only — PyTorch không thấy CUDA (kiểm tra torch+cu121)"
