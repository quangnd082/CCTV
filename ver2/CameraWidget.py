...
class VideoCapture:
    ...
    def __init__(self, name, target_width=None, logger: Logger = None):
        self.logger = logger
        self.source = name
        ...
        if isinstance(self.source, str) and self.source.lower().startswith("rtsp") and FFmpegCapture.is_available():
            try:
                # Scale sớm tại ingest (img_size) — giảm CPU decode/copy trước YOLO
...
class CameraWidget(QWidget):
    ...
    def __init__(self, camera_name="None", camera_src=0, img_size=640, yolo_model_path="", yolo_rate=0.5,
                 ...
                 is_heltmet_check=False, is_jacket_check=False, timer_delay=10, stream_enabled=False,
                 parent=None, logger: Logger = None):
        ...
        if stream_enabled and camera_name not in ("", "None", "—"):
            self.set_stream_enabled(True)
        else:
            self._set_stream_idle_ui()
    ...
    def set_stream_enabled(self, enabled: bool) -> None:
        """Bật/tắt capture + inference (điều khiển từ tab Settings / MainWindow)."""
    ...
    def bind_from_camera_info(self, cam_info, stream_enabled: bool = True) -> None:
        """Gán cấu hình camera cho ô Monitor (đổi nguồn RTSP / rule detect)."""
        self._stop_stream()

        flags = getattr(cam_info, "enable_flags", None) or {}
        self.camera_name = cam_info.camera_name
        self.camera_src = cam_info.camera_src
        self.img_size = cam_info.img_size
        self.yolo_model_path = cam_info.yolo_model_path
        self.yolo_rate = cam_info.yolo_rate
        self.classes = cam_info.classes
        self.colors = tuple(map(tuple, cam_info.colors))
        self.roi_check = cam_info.roi_check
        self.is_fell_check = bool(int(flags.get("fell", cam_info.is_fell_check)) == 1)
        self.is_helmet_check = bool(int(flags.get("helmet", cam_info.is_helmet_check)) == 1)
        self.is_jacket_check = bool(int(flags.get("jacket", cam_info.is_jacket_check)) == 1)
        self.is_fire_check = bool(int(flags.get("fire", cam_info.is_fire_check)) == 1)
        self.is_smoke_check = bool(int(flags.get("smoke", cam_info.is_smoke_check)) == 1)
        self.enable_flags = {
            "fell": self.is_fell_check,
            "helmet": self.is_helmet_check,
            "jacket": self.is_jacket_check,
        ...
        self.camera_name_label.setText(self.camera_name)
        self.checkbox_fell.setChecked(self.is_fell_check)
        self.checkbox_helmet.setChecked(self.is_helmet_check)
        self.checkbox_jacket.setChecked(self.is_jacket_check)
        self.checkbox_fire.setChecked(self.is_fire_check)
    ...
    def show_empty_slot(self, slot_label: str) -> None:
        """Ô Monitor chưa được gán camera (khi chọn < 4 cam)."""
        self._stop_stream()
        self.camera_name = slot_label
        self.camera_src = ""
        self.camera_name_label.setText(slot_label)
        try:
            self.image_label.clear()
        except Exception:
            pass
        if hasattr(self, "label_no_signal") and self.label_no_signal:
            self.label_no_signal.setText("CHƯA CHỌN")
            self.label_no_signal.setVisible(True)
        if hasattr(self, "status_text") and self.status_text:
            self.status_dot.setStyleSheet("background-color: #505050; border-radius: 5px;")
            self.status_text.setText("EMPTY")
            self.status_text.setStyleSheet("color: #707070; font-weight: bold; font-size: 12px;")
        self._has_signal = False

    def _start_stream(self) -> None:
        if not self.camera_src:
            return
        if self._stream_enabled and self.video_capture is not None:
            return
        self._stream_enabled = True
        self._last_frame_ts = 0.0
        self._infer_in_flight = False
        self.video_capture = VideoCapture(self.camera_src, target_width=self.img_size, logger=self.logger)
        if self.logger:
            self.logger.info(f"[CAM] stream ON: {self.camera_name} -> {self.camera_src}")
        self.label_no_signal.setText("NO SIGNAL")
    ...
    def _stop_stream(self) -> None:
        ...
        if self.video_capture is not None:
            ...
            self.video_capture = None
        if self.logger:
            self.logger.info(f"[CAM] stream OFF: {self.camera_name}")
        try:
            self.image_label.clear()
        except Exception:
            pass
    ...
    def _set_stream_idle_ui(self) -> None:
        """UI khi stream tắt."""
        try:
            ...
            if hasattr(self, "status_text") and self.status_text:
                ...
                self.status_text.setStyleSheet("color: #a0a0a0; font-weight: bold; font-size: 12px;")
            if hasattr(self, "label_no_signal") and self.label_no_signal:
                if self.camera_src:
                    self.label_no_signal.setText("OFF")
                else:
                    self.label_no_signal.setText("CHƯA CHỌN")
                self.label_no_signal.setVisible(True)
        except Exception:
    ...
    def _on_frame_timer(self):
        ...
        if not self._stream_enabled or self.video_capture is None:
            return
    ...
    def _on_infer_result(self, camera_name: str, q_image: QImage, is_warning: bool, meta: dict):
        ...
        if camera_name != self.camera_name or not self._stream_enabled:
            return
