...
from config_data import load_config_file, save_config_file
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.max_active_cameras = max(1, int(getattr(self.config_info, "max_active_cameras", 4) or 4))

        self.monitor_columns = max(1, min(2, int(getattr(self.config_info, "max_column", 2) or 2)))
    ...
    def _stop_all_streams(self) -> None:
        ...
        self.label_active_streams.setText(
            ...
            f"Đã dừng tất cả luồng — chọn camera và bấm «Xác nhận và áp dụng»"
    ...
    def _apply_selection(self, selected_names: list) -> None:
        ...
        self._persist_selection_flags(selected_names)
    ...
    def _persist_selection_flags(self, selected_names: list) -> None:

        sel = set(selected_names)

        for ci in self.config_info.camera_infos:

            if not isinstance(ci.enable_flags, dict):

                ci.enable_flags = {}

            ci.enable_flags["use_camera"] = 1 if ci.camera_name in sel else 0

        try:

            save_config_file(self.config_info, "config_data.json")

        except Exception as e:

            if self.logger:

                self.logger.warning(f"[CAM] could not save config: {e}")
    ...
    def create_log_widgets(self) -> None:
        ...
        self._apply_selection(self.settings_panel.get_selected_names())
        self._ui_ready = True
